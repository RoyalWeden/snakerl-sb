...
def display_all(screen, snake_head, snake_body, apple, score, object_shape:tuple):
    screen.fill((0,0,0))
...
class SnakeEnv(gym.Env):
    ...
    def __init__(self, snake_speed_time=0.0001, game_shape:tuple=(50,50), auto_update_shape:bool=False):
        ...
        self.observation_space = spaces.Box(low=-500, high=500, shape=(11 + SNAKE_LEN_GOAL,), dtype=np.float32)
    ...
    def step(self, action):
        ...
        display_all(self.screen, self.snake_head, self.snake_body, self.apple, self.score, self.object_shape)
        
        apple_reward = 0
        if hit_apple:
            apple_reward = 5 * self.score

        # increase game size when taking up most of map
        if len(self.snake_body) >= 0.8*(self.game_shape[0]*self.game_shape[1]) and self.auto_update_shape:
            self.game_shape = (self.game_shape[0]+1, self.game_shape[1]+1)
            self.object_shape = (pixel_shape['width']//self.game_shape[0], pixel_shape['height']//self.game_shape[1])

        # calcluate distance to apple and add reward
        euclidean_dist_to_apple = np.linalg.norm(np.array(self.snake_head) - np.array(self.apple))
        self.total_reward = ((1 - (euclidean_dist_to_apple / np.sqrt(pixel_shape['width']**2+pixel_shape['height']**2)))/2 + apple_reward)
        ...
        tail_x = self.snake_body[-1][0] // self.object_shape[0]
        tail_y = self.snake_body[-1][1] // self.object_shape[1]
        ...
        observation = [head_x, head_y, tail_x, tail_y, apple_delta_x, apple_delta_y, snake_length, wall_delta_left, wall_delta_up,
                    wall_delta_down, wall_delta_right] + list(self.prev_actions)
